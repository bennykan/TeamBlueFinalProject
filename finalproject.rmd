...
```{r, message=FALSE,warning=FALSE}
nyc_data$BUILDING.CLASS.CATEGORY[nyc_data$BUILDING.CLASS.CATEGORY=="1A"]<-1
nyc_data$BUILDING.CLASS.CATEGORY[nyc_data$BUILDING.CLASS.CATEGORY=="1B"]<-1
nyc_data$BUILDING.CLASS.CATEGORY[nyc_data$BUILDING.CLASS.CATEGORY=="1C"]<-1
nyc_data$BUILDING.CLASS.CATEGORY[nyc_data$BUILDING.CLASS.CATEGORY=="2A"]<-2
nyc_data$BUILDING.CLASS.CATEGORY[nyc_data$BUILDING.CLASS.CATEGORY=="2B"]<-2
nyc_data$BUILDING.CLASS.CATEGORY[nyc_data$BUILDING.CLASS.CATEGORY=="2C"]<-2
...
So from the above we can see that there are many values with low sale price.
```{r, message=FALSE,warning=FALSE}
...
ggplot(data = nyc_data, aes(x = log(nyc_data$LAND.SQUARE.FEET), y = log(nyc_data$SALE.PRICE), color = `BOROUGH`)) +
  geom_jitter() +
...
Let us try to find out how the sale price is per gross sq. feet.
```{r, message=FALSE,warning=FALSE}
ggplot(data = nyc_data, aes(x = log(nyc_data$GROSS.SQUARE.FEET), y = log(nyc_data$SALE.PRICE), color = `BOROUGH`)) +
  geom_jitter() +
  geom_smooth(method = "lm") +
  facet_wrap(~ BOROUGH) +
