...
###Boroughs
Let us see which borough recorded the the most property sales
```{r, message=FALSE,warning=FALSE}
...
```{r, message=FALSE,warning=FALSE}
dfnyc<-as.data.frame(table(nyc_data$BOROUGH, nyc_data$NEIGHBORHOOD))
names(dfnyc) <- c('BOROUGH','NEIGHBORHOOD', 'Freq')
dfnyc <- dfnyc %>% arrange(desc(Freq)) %>% head(10)
p3 <- ggplot(dfnyc, aes(x = 'NEIGHBORHOOD', y = 'Freq', fill = 'BOROUGH')) +
  geom_bar(stat = "identity") +
  coord_flip() +
...
table(encod_data$BOROUGH,data_out$BOROUGH)
