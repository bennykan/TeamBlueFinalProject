...
We can see that there are 16181 instances with NAs and we will remove these for the time being to find the relationship between the square footage and the sale price. For this purpose only we will create a new data frame "nyc_data_sqfeet" and we will plot the graphs using this dataframe.
...
We can see that there are 22918 instances with NA as value and we will remove these for the time being in order to find the relationship between the square footage and the sale price. For this purpose only we will create a new data frame "nyc_data_grosssqfeet" and we will plot the graphs using this dataframe.
...
We will now remove the outliers for the two square feet variables.

```{r, message=FALSE,warning=FALSE}
data_rm_outliers_sq_feet <- data

remove_outliers_sq_feet <- function(x,lbound) {
  qnt <- quantile(x, probs=c(.25, .75), na.rm = TRUE)
  H <- IQR(x, na.rm = TRUE)
  y=x
  y[x < (qnt[1] - H/lbound)] = -999999
...
data$GROSS.SQUARE.FEET <- remove_outliers_sq_feet(data_rm_outliers_sq_feet$GROSS.SQUARE.FEET,4)
data$LAND.SQUARE.FEET <- remove_outliers_sq_feet(data_rm_outliers_sq_feet$LAND.SQUARE.FEET,4)

data <- data[data$GROSS.SQUARE.FEET!=-999999&data$LAND.SQUARE.FEET!=-999999,]

```

The dataset now has 36762 elements.
