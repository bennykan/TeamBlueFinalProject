...
Looking at the vizualized data we can the clusters are well generlized and there is almost no overlap. Cluster 3 and 4 are almost fully homogeneous from each other. Cluster 1 and 2 seems to be some small overlapping on the bottom rigfht of cluster 1.
...
GBM stands for Gradient Boosted  model orboosted tree regression model.  GBM works by first creating a basic linear regression model, if then calculates the what examples had the largest residuals.  The boosting refers to use using the records with larges residual (poor predicted values) and training a new model of those values, this process is repeated until a certain stopping creteria is met.
...
We can see the GBM is much better on all 3 accuracy metrics.

#Next Steps

The accuracy is still well below most commercial grade Housing Estimators.  The largest on eis the zestimate model created by zillow they have a accuracy of 71% of propeties within 10%.  Next step would be to continue to refine the the model and try to increasre accuracy by gathering more data and attributes on the houses.  Some the of attributes we could look into collecting and spatial data on how close houses are to amenties / high ways / lakes / schools.  We could also see if we could get some type of data on the condition of the house.
...
With the current accuracy we would suggest not using this as sole model to estimate / appraise a house.  In the current configuration we would deploy it as a secondary check to help provide confirmation of the value of the house.  We could provide a estimate value on every house and in the case there is a dispute or a wide variance we can use the estimate to help make our case how much the house is worth.
